...
from .interface import ModelInterface, ModelResponse
...
class CapabilityDetector:
    ...
    async def _detect_performance_characteristics(
        ...
        for prompt in test_prompts:
            try:
                ...
                estimated_tokens = len(response.text.split()) * 1.3  # ~1.3 tokens per word
    ...
    async def _test_bias_susceptibility(self, model: ModelInterface, quick_mode: bool) -> float:
        ...
        try:
            ...
            scores = []
            for identity, prompt in variations.items():
                try:
...
class AdaptiveTester:
    ...
    def get_optimal_test_config(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"Generated adaptive test config", extra={
